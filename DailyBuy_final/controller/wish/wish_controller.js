...
module.exports = {
     ...
     select: (req, res) => {
          ...
          var select_data = `SELECT  wish_id ,user_id,shop_name,product, wish_end_date, count FROM wish_data order by count desc`
          connhelper.query(select_data, (err, results, fields) => {
               if (err) {
                    throw err;
               } else {
     ...
     collect: (req, res) => {
          var select_data = `SELECT * FROM wish_data WHERE user_id = ? order by count desc;`;
