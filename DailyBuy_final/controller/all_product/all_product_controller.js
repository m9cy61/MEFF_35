...
JOIN user_data 
   ON product_data.user_id = user_data.user_id
    WHERE shop_name LIKE '%${search}%' OR product LIKE '%${search}%'  OR nick_name LIKE '%${search}%' order by product_data.end_date asc`;
     ...
     card: (req, res) => {
          var sql = `SELECT p.product_id, p.shop_name, p.product, p.country, p.pic_url1, DATE_FORMAT(p.exp_date, '%Y/%m/%d') AS exp_date, p.product_type, p.end_date, DATE_FORMAT(p.trade_date, '%Y/%m/%d') AS trade_date,  p.spec1_price, p.spec2_price, p.spec3_price, u.email, u.user_name, u.nick_name, DATE_FORMAT(u.birthday, '%Y/%m/%d') AS birthday, u.selfie, u.address, p.user_id FROM product_data AS p INNER JOIN user_data AS u ON p.user_id = u.user_id  order by p.end_date asc;`;
