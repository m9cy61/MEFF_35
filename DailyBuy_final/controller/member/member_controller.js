...
module.exports = {
     ...
     follower: (req, res) => {
          ...
          var sql = "SELECT `order_data`.`user_id`,`order_data`.`product_id`,shop_name,product,pic_url1,DATE_FORMAT(end_date, '%Y-%m-%d') AS end_date,shipping_status FROM `order_data` JOIN `product_data` ON `order_data`.`product_id` = `product_data`.`product_id` WHERE `order_data`.`user_id` = ? ORDER BY end_date;"
     ...
     leader: (req, res) => {
          ...
          var sql = "SELECT product_id,shop_name,pic_url1,product,DATE_FORMAT(end_date, '%Y-%m-%d') AS end_date FROM `product_data` WHERE user_id = ? ORDER BY end_date;"
          connhelper.query(sql, req.session.user.id, (err, results, fields) => {
               res.json(results);
          })
     },
